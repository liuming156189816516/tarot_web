class JiQi {
	...
	constructor(arg) {
		...
		this.VE = function(yy) {
			...
			if (yx >= 1000 && yx <= 8001) {
				var m = (yx - 2000) / 1000;
				jdve = 2451623.80984 + 365242.37404 * m + 0.05169 * m * m - 0.00411 * m * m * m - 0.00057 * m *
					m * m * m;
			} else {
				if (yx >= -8000 && yx < 1000) {
					m = yx / 1000;
					jdve = 1721139.29189 + 365242.1374 * m + 0.06134 * m * m + 0.00111 * m * m * m - 0.00071 *
						m * m * m * m;
				} else {
					// 超出計算能力範圍
					return false;
		...
		this.Perturbation = function(jdez) {
			...
			var s = 0;
			var ptsa = new Array(485, 203, 199, 182, 156, 136, 77, 74, 70, 58, 52, 50, 45, 44, 29, 18, 17, 16,
				14, 12, 12, 12, 9, 8);
			var ptsb = new Array(
				324.96,
				337.23,
			...
			var l = 1 + 0.0334 * Math.cos((w * 2 * Math.PI) / 360) + 0.0007 * Math.cos((2 * w * 2 * Math.PI) /
				360);
			var ptb = (0.00001 * s) / l;
			return ptb;
		};
				...
				if (y < 500) {
					...
					if (y < 1600) {
						...
						if (y < 1700) {
							...
							if (y < 1800) {
								t = y - 1700;
								dt = 8.83 + 0.1603 * t - 0.0059285 * t * t + 0.00013336 * t * t * t - (t * t *
									t * t) / 1174000;
							} else {
								if (y < 1860) {
									t = y - 1800;
									...
									if (y < 1900) {
										...
										if (y < 1920) {
											t = y - 1900;
											dt = -2.79 + 1.494119 * t - 0.0598939 * t * t + 0.0061966 * t * t *
												t - 0.000197 * t * t * t * t;
										} else {
											if (y < 1941) {
												t = y - 1920;
												dt = 21.2 + 0.84493 * t - 0.0761 * t * t + 0.0020936 * t * t *
												t;
											} else {
												if (y < 1961) {
													t = y - 1950;
													...
													if (y < 1986) {
														t = y - 1975;
														dt = 45.45 + 1.067 * t - (t * t) / 260 - (t * t * t) /
															718;
													} else {
														if (y < 2005) {
															t = y - 2000;
		...
		this.MeanJQJD = function(yy, jdve, ty, ini, num) {
			...
			var tt = yy / 1000;
			var vp = 111.25586939 - 17.0119934518333 * tt - 0.044091890166673 * tt * tt - 4.37356166661345e-4 *
				tt * tt * tt + 8.16716666602386e-6 * tt * tt * tt * tt;
